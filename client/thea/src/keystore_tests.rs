...
use thea_primitives::{crypto, KEY_TYPE};

use crate::{error::Error, keystore::TheaKeystore};

/// Set of test accounts using [`thea_primitives::crypto`] types.
#[allow(missing_docs)]
...
fn authority_id_works() {
	...
	let store: TheaKeystore = Some(store).into();
...
#[test]
fn sign_no_keystore() {
	let store: TheaKeystore = None.into();
...
fn verify_works() {
	...
	assert!(TheaKeystore::verify(&alice, &sig, msg));
