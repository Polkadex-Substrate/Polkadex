...
use crate::Error;
use crate::utils::{add_balance, sub_balance};
...
fn test_add_balance_creates_new_free_balance() {
    ...
    assert_eq!(add_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(1_000_000_000_000u128).unwrap()), Ok(()));
...
fn test_add_balance_updates_existing_balance() {
    ...
    let new_account = AccountId::new([1;32]);
    let account_asset = AccountAsset { main: new_account, asset: AssetId::polkadex };
    add_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(1_000_000_000_000u128).unwrap()).unwrap();
    let add_amount = 1_000_000_000_000u128;
    assert_eq!(add_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(add_amount).unwrap()), Ok(()));
    let actual_balance = get_balance(&mut db_client, account_asset);
...
fn test_sub_balance_updates_balance() {
    ...
    let new_account = AccountId::new([1;32]);
    let account_asset = AccountAsset { main: new_account, asset: AssetId::polkadex };
    add_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(2_000_000_000_000u128).unwrap()).unwrap();
    let reduce_balance = 1_000_000_000_000u128;
    assert_eq!(sub_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(reduce_balance).unwrap()), Ok(()));
    let actual_balance = get_balance(&mut db_client, account_asset);
    let expected_balance = Decimal::from_u128(1_000_000_000_000u128).unwrap();
    assert_eq!(actual_balance, expected_balance);
}
...
fn test_sub_balance_returns_account_not_found() {
    ...
    let account_asset = AccountAsset { main: new_account, asset: AssetId::polkadex };
    let reduce_balance = 1_000_000_000_000u128;
    assert_eq!(sub_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(reduce_balance).unwrap()), Err(Error::AccountBalanceNotFound(account_asset)));
}
...
fn test_sub_balance_returns_insufficient_balance() {
    ...
    assert_eq!(sub_balance(&mut db_client, account_asset.clone(), Decimal::from_u128(reduce_balance).unwrap()), Err(Error::InsufficientBalance));
}

fn get_trie_db_client<'a>(memory_db: &'a mut MemoryDB<RefHasher, HashKey<RefHasher>, Vec<u8>>, working_state_root: &'a mut [u8;32]) -> TrieDBMut<'a, ExtensionLayout> {
