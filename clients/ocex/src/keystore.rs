...
use ocex_primitives::{
    crypto::{Public as OcexPublic, Signature},
    KEY_TYPE,
};
...
/// A Ocex specific keystore implemented as a `Newtype`. This is basically a
/// wrapper around [`sp_keystore::SyncCryptoStore`] and allows to customize
/// common cryptographic functionality.
#[derive(Clone)]
pub struct OcexKeystore(Option<SyncCryptoStorePtr>);
...
impl OcexKeystore {
    ...
    pub fn authority_id(&self, keys: &[OcexPublic]) -> Option<OcexPublic> {
        let store = self.0.clone()?;

        // we do check for multiple private keys as a key store sanity check.
        let public: Vec<OcexPublic> = keys
            .iter()
    ...
    pub fn sign(&self, public: &OcexPublic, message: &[u8]) -> Result<Signature, error::Error> {
    ...
    /// Returns a vector of [`ocex_primitives::crypto::Public`] keys which are currently supported
    /// (i.e. found in the keystore).
    pub fn public_keys(&self) -> Result<Vec<OcexPublic>, error::Error> {
        let store = self.0.clone().ok_or_else(|| error::Error::Keystore("no Keystore".into()))?;

        let pk: Vec<OcexPublic> = SyncCryptoStore::sr25519_public_keys(&*store, KEY_TYPE)
            .iter()
            .map(|k| OcexPublic::from(*k))
            .collect();
    ...
    pub fn verify(public: &OcexPublic, sig: &Signature, message: &[u8]) -> bool {
        let msg = keccak_256(message);
        let sig = sig.as_ref();
...
impl From<Option<SyncCryptoStorePtr>> for OcexKeystore {
    fn from(store: Option<SyncCryptoStorePtr>) -> OcexKeystore {
        OcexKeystore(store)
    }
}
