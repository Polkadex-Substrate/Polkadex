...
mod uniswap_v2 {
	...
	/// Emitted when Adding liquidity success. \[who, currency_id_0, pool_0_increment,
	...
	/// Emitted when Removing liquidity from the trading pool success. \[who, currency_id_0,
	...
	/// Use supply currency to swap target currency. \[trader, trading_path, supply_currency_amount,
	...
	impl UniswapV2 {
		...
		fn _swap(
				...
				TradingPair::from_currency_ids(supply_currency_id, target_currency_id)
			{
				...
				if supply_currency_id == trading_pair.first() {
					let pool_0 =
						pool_0.checked_add(supply_increment).ok_or(Error::ArithmeticOverflow)?;
					let pool_1 =
						pool_1.checked_sub(target_decrement).ok_or(Error::ArithmeticUnderflow)?;
					self.liquidity_pool.insert(trading_pair.clone(), (pool_0, pool_1));
				} else {
					let pool_0 =
						pool_0.checked_sub(target_decrement).ok_or(Error::ArithmeticUnderflow)?;
					let pool_1 =
						pool_1.checked_add(supply_increment).ok_or(Error::ArithmeticOverflow)?;
					self.liquidity_pool.insert(trading_pair.clone(), (pool_0, pool_1));
				}
		...
		pub fn do_swap_with_exact_supply(
			...
			self.env().extension().deposit(path[0], caller, supply_amount)?;
		...
		pub fn do_swap_with_exact_target(
			...
			self.env().extension().deposit(path[0], caller, actual_supply_amount)?;
			self._swap_by_path(&path, &amounts)?;
			self.env().extension().withdraw(path[path.len() - 1], caller, target_amount)?;
		...
		fn do_deposit_dex_share(
			...
			let incentives =
				incentives.checked_add(share_increment).ok_or(Error::ArithmeticOverflow)?;

			self.dex_incentives.insert((trading_pair.clone(), who), incentives);

			let (_, total_shares) = self.pair_info(&trading_pair);

			let total_shares =
				total_shares.checked_add(share_increment).ok_or(Error::ArithmeticOverflow)?;

			self.total_issuances.insert(trading_pair.clone(), total_shares);
		...
		fn do_withdraw_dex_share(
			...
			let incentives =
				incentives.checked_sub(share_decrement).ok_or(Error::ArithmeticOverflow)?;
			...
			let total_shares =
				total_shares.checked_sub(share_decrement).ok_or(Error::ArithmeticOverflow)?;
		...
		fn do_deposit_pool(
			...
			let ((pool_0, pool_1), _) = self.pair_info(&trading_pair);
			let pool_0 = pool_0.checked_add(pool_0_increment).ok_or(Error::ArithmeticOverflow)?;
			let pool_1 = pool_1.checked_add(pool_1_increment).ok_or(Error::ArithmeticOverflow)?;
		...
		fn do_withdraw_pool(
			...
			let ((pool_0, pool_1), _) = self.pair_info(&trading_pair);
			let pool_0 = pool_0.checked_sub(pool_0_decrement).ok_or(Error::ArithmeticOverflow)?;
			let pool_1 = pool_1.checked_sub(pool_1_decrement).ok_or(Error::ArithmeticOverflow)?;
		...
		fn do_add_liquidity(
			...
			self.env().extension().deposit(trading_pair.first(), caller, pool_0_increment)?;
			self.env()
				.extension()
		...
		fn do_remove_liquidity(
			...
			let pool_0_decrement =
				proportion.checked_mul_int(pool_0).ok_or(Error::ArithmeticOverflow)?.to_num();
			let pool_1_decrement =
				proportion.checked_mul_int(pool_1).ok_or(Error::ArithmeticOverflow)?.to_num();
