...
pub mod pallet {
    ...
    type LMPScoreSheet<T> = BTreeMap<
        ...
        (
            BTreeMap<<T as frame_system::Config>::AccountId, (BalanceOf<T>, bool)>,
            BalanceOf<T>,
        ),
    >;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_pool(
            ...
            let market_maker = ensure_signed(origin)?;
                ...
                !<Pools<T>>::contains_key(market, &market_maker),
                Error::<T>::PoolExists
            );
            // Check market is active
            ensure!(
                T::OCEX::is_registered_market(&market),
                Error::<T>::UnknownMarket
            );
        ...
        pub fn add_liquidity(
            ...
            let config = <Pools<T>>::get(market, &market_maker).ok_or(Error::<T>::UnknownPool)?;
                ...
                <SnapshotFlag<T>>::get().is_none(),
                Error::<T>::SnapshotInProgress
            ); // TODO: @zktony Replace with pool level flags
            ...
            let required_quote_amount = average_price.saturating_mul(base_amount);
                ...
                required_quote_amount <= max_quote_amount,
                Error::<T>::NotEnoughQuoteAmount
            );
