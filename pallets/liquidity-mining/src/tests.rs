...
use orderbook_primitives::constants::UNIT_BALANCE;
...
fn test_register_pool_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(LiqudityMining::register_pool(RuntimeOrigin::signed(market_maker.clone()), name, trading_pair, commission, exit_fee, public_fund_allowed, trading_account.clone()));
        // Verification
        assert!(LiqudityMining::lmp_pool(trading_pair, market_maker.clone()).is_some());
        assert_noop!(LiqudityMining::register_pool(RuntimeOrigin::signed(market_maker.clone()), name, trading_pair, commission, exit_fee, public_fund_allowed, trading_account), crate::pallet::Error::<Test>::PoolExists);
    })
}
...
fn test_register_pool_error_unknown_pool() {
    new_test_ext().execute_with(|| {
        ...
        let market_maker = AccountId32::new([2;32]);
        mint_base_quote_asset_for_user(market_maker.clone());
        assert_noop!(LiqudityMining::register_pool(RuntimeOrigin::signed(market_maker.clone()), name, trading_pair, commission, exit_fee, public_fund_allowed, trading_account.clone()), crate::pallet::Error::<Test>::UnknownMarket);
    })
}
...
fn test_register_pool_error_register_pool_fails() {
    new_test_ext().execute_with(|| {
        ...
        register_test_trading_pair();
        mint_base_quote_asset_for_user(market_maker.clone());
        assert_noop!(LiqudityMining::register_pool(RuntimeOrigin::signed(market_maker.clone()), name, trading_pair, commission, exit_fee, public_fund_allowed, trading_account.clone()), pallet_ocex_lmp::pallet::Error::<Test>::ProxyAlreadyRegistered);
...
use frame_support::traits::fungibles::{Create, Mutate as MutateNonNative};
...
fn test_add_liquidity_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker, UNIT_BALANCE * 6, UNIT_BALANCE * 40));
        // * Check user balance
...
fn test_add_liquidity_error_public_fund_not_allowed() {
    new_test_ext().execute_with(|| {
        ...
        let quote_asset = AssetId::Asset(1);
        let trading_pair = TradingPair { base: base_asset, quote: quote_asset };
        assert_noop!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker, UNIT_BALANCE * 6, UNIT_BALANCE * 40), crate::pallet::Error::<Test>::PublicDepositsNotAllowed);
    })
}
...
fn test_add_liquidity_error_price_not_found() {
    new_test_ext().execute_with(|| {
        ...
        let quote_asset = AssetId::Asset(1);
        let trading_pair = TradingPair { base: base_asset, quote: quote_asset };
        assert_noop!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker, UNIT_BALANCE * 6, UNIT_BALANCE * 40), crate::pallet::Error::<Test>::PriceNotAvailable);
    })
}
...
fn test_add_liquidity_error_not_enough_quote_amount() {
    new_test_ext().execute_with(|| {
        ...
        // Cretae Base and Quote Asset;
        mint_base_quote_asset_for_user(user_who_wants_to_add_liq.clone());
        assert_noop!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker, UNIT_BALANCE * 6, UNIT_BALANCE * 10), crate::pallet::Error::<Test>::NotEnoughQuoteAmount);
    })
}
...
fn test_add_liquidity_not_enough_token_to_trasnfer() {
    new_test_ext().execute_with(|| {
        ...
        // Cretae Base and Quote Asset;
        mint_base_quote_asset_for_user(user_who_wants_to_add_liq.clone());
        assert_noop!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker, UNIT_BALANCE * 10000, UNIT_BALANCE * 40000000), Underflow);
    })
}
...
fn test_remove_liquidity_happy_path_and_error() {
    new_test_ext().execute_with(|| {
        ...
        let quote_asset = AssetId::Asset(1);
        let trading_pair = TradingPair { base: base_asset, quote: quote_asset };
        assert_ok!(LiqudityMining::remove_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker.clone(), UNIT_BALANCE * 6));
        let (pool, share_id) = LiqudityMining::create_pool_account(&market_maker, trading_pair);
        // * Check shares of user
        assert_eq!(Assets::balance(share_id, &user_who_wants_to_add_liq), 0);
        assert_noop!(LiqudityMining::remove_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker.clone(), UNIT_BALANCE * 6), crate::pallet::Error::<Test>::TotalShareIssuanceIsZero);
    })
}
...
fn test_force_close_pool_happy_path_and_error() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(LiqudityMining::pool_force_close_success(trading_pair, &market_maker, base_freed, quote_freed));
        assert_ok!(LiqudityMining::force_close_pool(RuntimeOrigin::root(), trading_pair, market_maker.clone()));
...
fn test_add_liquidity_success_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(LiqudityMining::add_liquidity_success(trading_pair, &market_maker, &lp, share_issued, price, total_inventory_in_quote));
    })
}
#[test]
fn test_submit_scores_of_lps_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        let mut results: BTreeMap<
            ...
            (BTreeMap<AccountId32, (u128, bool)>, u128)> = BTreeMap::new();
        results.insert((TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, market_maker.clone(), 1), (score_map, total_score));
        register_test_pool(true);
        assert_ok!(LiqudityMining::submit_scores_of_lps(RuntimeOrigin::none(), results));
    })
}

use orderbook_primitives::{TradingPairMetricsMap, TradingPairMetrics, TraderMetricsMap};
...
fn test_claim_rewards_by_lp_happy_path_and_error() {
    new_test_ext().execute_with(|| {
        ...
        add_lmp_config();
        update_lmp_score();
        let reward_account = <crate::mock::Test as pallet_ocex_lmp::Config>::LMPRewardsPalletId::get().into_account_truncating();
        ...
        results.insert((TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, market_maker.clone(), 1), (score_map, total_score));
        assert_ok!(LiqudityMining::submit_scores_of_lps(RuntimeOrigin::none(), results));
        assert_ok!(LiqudityMining::claim_rewards_by_lp(RuntimeOrigin::signed(trader.clone()), TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, market_maker.clone(), 1 ));
        assert_noop!(LiqudityMining::claim_rewards_by_lp(RuntimeOrigin::signed(trader.clone()), TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, market_maker.clone(), 1 ), crate::pallet::Error::<Test>::AlreadyClaimed);
    })
}
...
fn test_claim_rewards_by_mm_happy_path_and_error() {
    new_test_ext().execute_with(|| {
        ...
        results.insert((TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, market_maker.clone(), 1), (score_map, total_score));
        assert_ok!(LiqudityMining::submit_scores_of_lps(RuntimeOrigin::none(), results));
        assert_ok!(LiqudityMining::claim_rewards_by_mm(RuntimeOrigin::signed(market_maker.clone()), TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, 1 ));
        assert_noop!(LiqudityMining::claim_rewards_by_mm(RuntimeOrigin::signed(market_maker.clone()), TradingPair { base: AssetId::Polkadex, quote: AssetId::Asset(1) }, 1 ), crate::pallet::Error::<Test>::AlreadyClaimed);
    })
}
...
fn test_initiate_withdrawal() {
    new_test_ext().execute_with(|| {
        ...
        // Remove liquidity
        assert_ok!(LiqudityMining::initiate_withdrawal(RuntimeOrigin::signed(market_maker), trading_pair, epoch, num_of_request));
    })
}
...
fn add_liquidity() {
    ...
    // Cretae Base and Quote Asset;
    mint_base_quote_asset_for_user(user_who_wants_to_add_liq.clone());
    assert_ok!(LiqudityMining::add_liquidity(RuntimeOrigin::signed(user_who_wants_to_add_liq.clone()), trading_pair, market_maker.clone(), UNIT_BALANCE * 6, UNIT_BALANCE * 40));
    let (pool, share_id) = LiqudityMining::create_pool_account(&market_maker, trading_pair);
    let share_issued = Decimal::from(6);
    let price = Decimal::from(5);
    let total_inventory_in_quote = Decimal::from(40);
    assert_ok!(LiqudityMining::add_liquidity_success(trading_pair, &market_maker, &user_who_wants_to_add_liq, share_issued, price, total_inventory_in_quote));
}

fn mint_base_quote_asset_for_user(user: AccountId32) {
    let quote_asset = AssetId::Asset(1);
    Balances::mint_into(&user, UNIT_BALANCE * 100);
    Assets::create(RuntimeOrigin::signed(user.clone()), parity_scale_codec::Compact(quote_asset.asset_id().unwrap()), AccountId32::new([1;32]), One::one());
...
fn register_test_trading_pair() {
    ...
    assert_ok!(OCEX::set_exchange_state(RuntimeOrigin::root(), true));
    assert_ok!(OCEX::register_trading_pair(RuntimeOrigin::root(), base, quote, min_order_price, max_order_price, min_order_qty, max_order_qty, price_tick_size, qty_step_size));
}
