...
use polkadex_primitives::{assets::AssetId, BlockNumber};
use rand::{seq::SliceRandom, SeedableRng};
use rand_chacha::ChaChaRng;
use sp_arithmetic::traits::{Bounded, One, SaturatedConversion, Saturating, Zero, *};
use sp_core::H256;
use sp_runtime::traits::{DispatchInfoOf, SignedExtension};
use sp_std::{boxed::Box, collections::vec_deque::VecDeque, vec, vec::Vec};
...
impl<T: Config + frame_system::Config> Default for Stake<T> {
	fn default() -> Self {
		Stake { staked_amount: 0_u128.saturated_into(), unlocking_block: 1_u32.saturated_into() }
	}
}

impl<T: Config + frame_system::Config> Stake<T> {
	pub fn new(stake: T::Balance, unlock: T::BlockNumber) -> Stake<T> {
		Stake { staked_amount: stake, unlocking_block: unlock }
	}
}
...
impl<Call, Origin> Default for ExtStore<Call, Origin> {
	fn default() -> Self {
		Self { store: Vec::new(), total_weight: 0 }
	}
}
...
impl<T: Config> Module<T> {
	...
	// The staking period will vary between 28-56 days. The heavier the transaction, longer the
	// period. A transaction consuming the full allocated weight lock the tokens for 56 days.
	pub fn calculate_stake_params(
		...
		let mut stake_period: T::BlockNumber = T::MinStakePeriod::get().saturated_into(); // 28 days
																				  // Add a extra staking period based on the fraction of total weight allocation this call
																				  // occupies
...
impl<T: Config> DynamicStaking<T>
where
	<T as Config>::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	///
	/// This will try and optimise the `stake/weight` `stake/length`, whichever is consuming more of
	/// the maximum corresponding limit.
	///
	...
	/// and the entire block weight `(1/1)`, its priority is `stake * min(1, 4) = fee * 1`. This
	/// means  that the transaction which consumes more resources (either length or weight) with the
	/// same `fee` ends up having lower priority.
...
impl<T: Config> SignedExtension for DynamicStaking<T>
where
	<T as Config>::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	fn validate(
		...
		if T::DynamicStaking::filter(call) {
			...
			Ok(ValidTransaction {
				priority: Self::calculate_priority(len, info, stake),
				..Default::default()
			})
		} else {
