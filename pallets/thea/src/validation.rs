...
use crate::{
	aggregator::AggregatorClient, pallet::ActiveNetworks, resolver::Resolver, Config, Pallet,
};
...
impl<T: Config> Pallet<T> {
	...
	pub fn run_thea_validation(_blk: BlockNumberFor<T>) -> Result<(), &'static str> {
		...
		for network in active_networks {
			...
			log::debug!(target:"thea","Outgoing nonce: {:?} for network: {:?}",next_outgoing_nonce,network);
