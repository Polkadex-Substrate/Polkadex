...
fn test_session_change() {
    new_test_ext().execute_with(|| {
        ...
        let current_authorities: Vec<<Test as Config>::TheaId> = authorities
            .iter()
...
fn test_send_thea_message_bad_inputs() {
    new_test_ext().execute_with(|| {
        // bad origin
        assert_err!(
            Thea::send_thea_message(RuntimeOrigin::none(), vec!(), 0),
            BadOrigin
        );
        assert_err!(
            Thea::send_thea_message(RuntimeOrigin::signed(0), vec!(), 0),
            BadOrigin
        );
        assert_err!(
            Thea::send_thea_message(RuntimeOrigin::signed(1), vec!(), 0),
            BadOrigin
        );
        assert_err!(
        ...
        assert_err!(
            Thea::send_thea_message(RuntimeOrigin::signed(u64::MAX), vec!(), 0),
            BadOrigin
        );
...
fn test_update_incoming_nonce_all() {
    new_test_ext().execute_with(|| {
        // bad origins
        assert_err!(
            Thea::update_incoming_nonce(RuntimeOrigin::none(), u64::MAX, 0),
            BadOrigin
        );
        assert_err!(
            Thea::update_incoming_nonce(RuntimeOrigin::signed(1), u64::MAX, 0),
            BadOrigin
        );
        assert_err!(
        ...
        assert_ok!(Thea::update_incoming_nonce(
            RuntimeOrigin::root(),
            10_000,
            0
        ));
...
fn test_update_outgoing_nonce_all() {
    new_test_ext().execute_with(|| {
        // bad origins
        assert_err!(
            Thea::update_outgoing_nonce(RuntimeOrigin::none(), u64::MAX, 0),
            BadOrigin
        );
        assert_err!(
            Thea::update_outgoing_nonce(RuntimeOrigin::signed(1), u64::MAX, 0),
            BadOrigin
        );
        assert_err!(
        ...
        assert_ok!(Thea::update_outgoing_nonce(
            RuntimeOrigin::root(),
            10_000,
            0
        ));
...
fn test_remove_thea_network_full() {
    new_test_ext().execute_with(|| {
        // bad origins
        assert_err!(
            Thea::remove_thea_network(RuntimeOrigin::none(), 1),
            BadOrigin
        );
        assert_err!(
            Thea::remove_thea_network(RuntimeOrigin::signed(1), 1),
            BadOrigin
        );
...
fn test_handle_misbehaviour_happy_path_valid_proposal() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Thea::handle_misbehaviour(
            RuntimeOrigin::root(),
            network,
            1,
            true
...
fn test_submit_signed_outgoing_messages_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        auth.try_push(sp_core::ecdsa::Public::from_raw([1; 33]).into())
            .unwrap();
        auth.try_push(sp_core::ecdsa::Public::from_raw([2; 33]).into())
            .unwrap();
        let auth_index = 2;
...
fn test_validate_signed_outgoing_message_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        <OutgoingMessages<Test>>::insert(network, nonce, message);
        assert_ok!(Thea::validate_signed_outgoing_message(
            &0,
            &validator_set_id,
            &signatures
        ));
...
fn test_add_signature() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            signed_message.signatures.get(&0).unwrap().clone(),
            signature
        );
