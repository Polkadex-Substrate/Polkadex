...
fn allowlist_chain() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Bridge::allowlist_chain(RuntimeOrigin::root(), 0));
		assert_noop!(
			Bridge::allowlist_chain(RuntimeOrigin::root(), TestChainId::get()),
			Error::<Test>::InvalidChainId
		);

		assert_events(vec![RuntimeEvent::Bridge(PalletEvent::ChainAllowlisted(0))]);
	})
}
...
fn asset_transfer_success() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Bridge::allowlist_chain(RuntimeOrigin::root(), dest_id.clone()));
		...
		assert_events(vec![
			RuntimeEvent::Bridge(PalletEvent::ChainAllowlisted(dest_id.clone())),
		...
		assert_events(vec![RuntimeEvent::Bridge(PalletEvent::NonFungibleTransfer(
			dest_id.clone(),
			2,
			resource_id.clone(),
			token_id,
		...
		assert_events(vec![RuntimeEvent::Bridge(PalletEvent::GenericTransfer(
			dest_id.clone(),
			3,
			resource_id,
			metadata,
...
fn asset_transfer_invalid_chain() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Bridge::allowlist_chain(RuntimeOrigin::root(), chain_id.clone()));
		assert_events(vec![RuntimeEvent::Bridge(PalletEvent::ChainAllowlisted(chain_id.clone()))]);
