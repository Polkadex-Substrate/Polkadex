...
impl<T: Config> Pallet<T> {
        ...
        if sp_io::offchain::is_validator() {
            match available_keys.first() {
                ...
                Some(key) => {
                    ...
                    store_summary::<T>(summary, signature, auth_index.saturated_into());
    ...
    fn withdraw(
        ...
        let withdrawal = request
            .convert(stid)
            .map_err(|_| "Withdrawal conversion error")?;
        ...
        for action in &batch.actions {
            match action {
                ...
                UserActions::BlockImport(blk, engine_messages, price_oracle) => {
                    ...
                    egress_messages.push(EgressMessages::PriceOracle(price_oracle.clone()));
                    // nothing to verify
    ...
    pub fn compute_trader_metrics(
        ...
        if let Some(epoch) = <FinalizeLMPScore<T>>::get() {
            ...
            let mut scores_map: BTreeMap<
                TradingPair,
                (
                    BTreeMap<T::AccountId, (Decimal, Decimal)>,
                    (Decimal, Decimal),
                ),
            > = BTreeMap::new();
    ...
    pub fn get_offchain_balance(
        ...
        let balance: BTreeMap<AssetId, Decimal> = match state
            .get(account.as_slice())
            .map_err(crate::validator::map_trie_error)?
        {
            None => BTreeMap::new(),
            Some(encoded) => BTreeMap::decode(&mut &encoded[..])
                .map_err(|_| "Unable to decode balances for account")?,
