...
fn test_ocex_submit_snapshot() {
    ...
    new_test_ext().execute_with(|| {
        ...
        OCEX::validate_snapshot(
            &snapshot,
            &vec![
                (0, signature1.clone().into()),
                (1, signature2.clone().into()),
            ],
        )
        .unwrap();
...
fn test_add_balance_new_account() {
    ...
    ext.execute_with(|| {
        ...
        assert!(rs
            .1
            .get(&account_id)
            .is_some_and(|v| !v.is_empty() && v[0] == account_id));
        // balance present and correct
        let expected: Decimal = amount.into();
        assert_eq!(
            rs.2.get(&AccountAsset {
                main: account_id,
                asset: asset_id
            })
            .unwrap(),
...
fn test_add_balance_existing_account_with_balance() {
    ...
    ext.execute_with(|| {
        ...
        let account_info: BTreeMap<AssetId, Decimal> = BTreeMap::decode(&mut &encoded[..]).unwrap();
        assert_eq!(
            account_info.get(&asset_id).unwrap(),
            &(amount + amount2).into()
        );
        // test get_balance()
        state.commit().unwrap();
...
fn test_register_main_account() {
    ...
    new_test_ext().execute_with(|| {
        assert_ok!(OCEX::set_exchange_state(RuntimeOrigin::root(), true));
        assert_eq!(
            Accounts::<Test>::contains_key::<AccountId32>(account_id.clone().into()),
            false
        );
        assert_ok!(OCEX::register_main_account(
...
fn test_register_trading_pair() {
    new_test_ext().execute_with(|| {
        assert_ok!(OCEX::set_exchange_state(RuntimeOrigin::root(), true));
        assert_ok!(OCEX::allowlist_token(
            RuntimeOrigin::root(),
            AssetId::Asset(20)
        ));
        assert_ok!(OCEX::allowlist_token(
...
fn test_open_trading_pair() {
    new_test_ext().execute_with(|| {
        ...
        assert_last_event::<Test>(
            crate::Event::OpenTradingPair {
                pair: trading_pair.clone(),
            }
            .into(),
        );
        let event: IngressMessages<AccountId32> = IngressMessages::OpenTradingPair(trading_pair);
...
fn test_submit_snapshot() {
    ...
    t.execute_with(|| {
        ...
        assert_eq!(
            Snapshots::<Test>::get(1).unwrap().withdrawals,
...
fn test_withdrawal_bad_origin() {
    new_test_ext().execute_with(|| {
        let account_id = create_account_id();
        assert_noop!(
            OCEX::claim_withdraw(RuntimeOrigin::root(), 1, account_id.clone()),
            BadOrigin
        );

        assert_noop!(
            OCEX::claim_withdraw(RuntimeOrigin::none(), 1, account_id.clone()),
            BadOrigin
        );
    });
}
...
pub fn test_allowlist_and_blacklist_token() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(OCEX::remove_allowlisted_token(
            RuntimeOrigin::root(),
            new_token
        ));
        let allowlisted_tokens = <AllowlistedToken<Test>>::get();
...
fn test_remove_proxy_account_faulty_cases() {
    ...
    new_test_ext().execute_with(|| {
        // bad origin
        assert_noop!(
            OCEX::remove_proxy_account(RuntimeOrigin::root(), proxy.clone()),
            BadOrigin
        );
        assert_noop!(
            OCEX::remove_proxy_account(RuntimeOrigin::none(), proxy.clone()),
            BadOrigin
        );
...
fn test_remove_proxy_account_proper_case() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(OCEX::remove_proxy_account(
            RuntimeOrigin::signed(main),
            proxy
        ));
    })
}
...
fn test_set_exchange_state_full() {
    new_test_ext().execute_with(|| {
        ...
        // bad origins
        assert_noop!(
            OCEX::set_exchange_state(RuntimeOrigin::none(), true),
            BadOrigin
        );
        assert_noop!(
            OCEX::set_exchange_state(RuntimeOrigin::signed(a), true),
            BadOrigin
        );
        assert_noop!(
            OCEX::set_exchange_state(RuntimeOrigin::signed(b), true),
            BadOrigin
        );
        // proper case
        assert_ok!(OCEX::set_exchange_state(RuntimeOrigin::root(), true));
...
fn test_whitelist_orderbook_operator_full() {
    new_test_ext().execute_with(|| {
        ...
        // bad origins
        assert_noop!(
            OCEX::whitelist_orderbook_operator(RuntimeOrigin::none(), key),
            BadOrigin
        );
        assert_noop!(
        ...
        // proper case
        assert_ok!(OCEX::whitelist_orderbook_operator(
            RuntimeOrigin::root(),
            key
        ));
...
fn test_do_claim_lmp_rewards_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(OCEX::do_claim_lmp_rewards(
            main_account.clone(),
            epoch,
            trading_pair
        ));
...
fn test_set_fee_distribution() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(OCEX::set_fee_distribution(
            RuntimeOrigin::root(),
            fee_distribution
        ));
    })
...
fn test_close_auction_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(&recipient_address),
            25 * UNIT_BALANCE
        );
        assert_eq!(Balances::free_balance(&bidder), 50 * UNIT_BALANCE);
        assert_eq!(
            Assets::balance(usdt_asset.asset_id().unwrap(), &bidder),
            10 * UNIT_BALANCE
        );
        assert_eq!(
            Assets::balance(usdc_asset.asset_id().unwrap(), &bidder),
            10 * UNIT_BALANCE
        );
        let pot_account = OCEX::get_pot_account();
        assert_eq!(
...
fn test_place_bid_happy_path() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(OCEX::place_bid(
            RuntimeOrigin::signed(bidder_two.clone()),
            bid_amount_two
        ));
        let actual_auction_info = <Auction<Test>>::get();
        let expected_auction_info = AuctionInfo {
...
fn create_order_by_alice(
    ...
    let fee_config = FeeConfig {
        maker_fraction: Default::default(),
        taker_fraction: Default::default(),
    };
