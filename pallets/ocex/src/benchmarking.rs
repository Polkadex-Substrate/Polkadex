...
benchmarks! {
	register_main_account {
		let b in 0 .. 50_000;
		let main: T::AccountId = account("mem1", b, SEED);
		let proxy: T::AccountId = account("mem1", b, SEED);
	...
	add_proxy_account {
		let b in 0 .. 255; // should not overflow u8
		let main: T::AccountId = account("mem1", b, SEED);
		let proxy: T::AccountId = account("mem1", b, SEED);
		<ExchangeState<T>>::put(true);
	...
	submit_snapshot {
		...
		let call = Call::<T>::submit_snapshot { summary: snapshot, signatures };
