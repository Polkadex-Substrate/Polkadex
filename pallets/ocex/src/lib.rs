...
use frame_support::{
	...
	traits::Get, PalletId, StorageMap,
};
...
decl_module! {
	...
	/// Removes Proxy Account for given Main Account.
		...
		/// Adds Referral Id for given Account.
		///
		/// # Parameter
		///
		/// * `account`: Account for whom Referral Id is to be added
		...
		/// Removes Referral Id for given Account.
		///
		/// # Parameter
		///
		/// * `account`: Account for whom Referral Id is to be removed
		#[weight = 10000]
...
impl<T: Config> Module<T> {
	...
	pub fn remove_proxy_(main: T::AccountId, proxy: T::AccountId) -> Result<(), Error<T>> {
		<MainAccounts<T>>::try_mutate(main.clone(), |ref mut linked_account: &mut LinkedAccount| {
			let index = linked_account
				.proxies
				.iter()
				.position(|x| <AccountId as AsRef<[u8]>>::as_ref(x) == proxy.encode().as_slice())
				.unwrap();
