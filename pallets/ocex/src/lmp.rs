...
impl<T: Config> LiquidityMining<T::AccountId, BalanceOf<T>> for Pallet<T> {
    ...
    fn add_liquidity(
        ...
        <IngressMessages<T>>::mutate(current_blk, |messages| {
            messages.push(
                orderbook_primitives::ingress::IngressMessages::AddLiquidity(
    ...
    fn remove_liquidity(
        ...
        <IngressMessages<T>>::mutate(current_blk, |messages| {
            messages.push(
                orderbook_primitives::ingress::IngressMessages::RemoveLiquidity(
                    TradingPairConfig::default(market.base, market.quote),
                    pool,
                    lp,
                    burn_frac,
    ...
    fn force_close_pool(market: TradingPair, pool: T::AccountId) {
        ...
        <IngressMessages<T>>::mutate(current_blk, |messages| {
            messages.push(
                orderbook_primitives::ingress::IngressMessages::ForceClosePool(
                    TradingPairConfig::default(market.base, market.quote),
                    pool,
                ),
            );
