	...
	pub fn calculate_inventory_deviation() -> Result<BTreeMap<AssetId, Decimal>, DispatchError> {
		...
		if let Ok(_) = Self::acquire_offchain_lock() {
			...
			for (main, _) in <Accounts<T>>::iter() {
				...
				let balances: BTreeMap<AssetId, Decimal> = Self::get_balances(
					&mut state,
			...
			for asset in &assets {
				let diff =
					onchain_inventory.get(asset).cloned().unwrap_or_default().saturating_sub(
						offchain_inventory.get(&asset).cloned().unwrap_or_default(),
					);
