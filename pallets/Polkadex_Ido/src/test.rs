...
use polkadex_primitives::assets::{AssetId, AssetId::POLKADEX};
use sp_core::H160;
...
fn test_withdraw_raise() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		<Test as Config>::Currency::deposit(AssetId::Asset(24), &4_u64, 100000);
...
fn test_get_reserve_amount() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let reserve_balance = <Test as Config>::Currency::total_balance(AssetId::POLKADEX, &4_u64) -
			<Test as Config>::Currency::free_balance(AssetId::POLKADEX, &4_u64);
