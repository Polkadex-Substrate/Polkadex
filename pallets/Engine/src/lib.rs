...
decl_storage! {
	trait Store for Module<T: Config> as Engine {
	    ...
	    Traders get(fn get_traders): map hasher(blake2_128_concat) T::AccountId => AccountData<T::Balance>;
	}
}
...
impl<T: Config> Module<T> {
    fn settle(provider: T::AccountId, maker: Order<T::Balance, T::AccountId, T::Signature>, taker: Order<T::Balance, T::AccountId, T::Signature>) -> Result<(), Error<T>> {
        ...
        if <Providers<T>>::contains_key(provider) {
            ...
            if Self::verify_signatures(&maker, &taker) {
                // Verify nonce
                let maker_account: AccountData<T::Balance> = <Traders<T>>::get(&maker.trader);
                let taker_account: AccountData<T::Balance> = <Traders<T>>::get(&taker.trader);
    ...
    fn verify_signatures(maker: &Order<T::Balance, T::AccountId, T::Signature>, taker: &Order<T::Balance, T::AccountId, T::Signature>) -> bool {
    ...
    fn verify_nonces(maker_account: &AccountData<T::Balance>, maker: &Order<T::Balance, T::AccountId, T::Signature>,
                     taker_account: &AccountData<T::Balance>, taker: &Order<T::Balance, T::AccountId, T::Signature>) -> bool {
        // FIXME: Implement an efficient nonce verification

        true
    }

    fn execute(maker_account: &AccountData<T::Balance>, maker: &Order<T::Balance, T::AccountId, T::Signature>,
               taker_account: &AccountData<T::Balance>, taker: &Order<T::Balance, T::AccountId, T::Signature>) -> Result<(), Error<T>> {
        // FIXME: Transfer the balances
        Ok(())
    }
}