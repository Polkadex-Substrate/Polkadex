...
fn test_claim_tokens_native_pdex_as_token_b() {
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            <Test as Config>::AssetManager::balance(24,&investor_address ),
            balance
        );
...
fn test_claim_edge_case_small_investment() {
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert!(<Test as Config>::AssetManager::increase_balance(
            24,
            &ALICE,
            amount,
        ).is_ok());
...
fn test_account_balances() {
    ExtBuilder::default().build().execute_with(|| {
        assert_eq!(PolkadexIdo::account_balances(vec![1,2, 3, 5], ALICE), vec![INITIAL_BALANCE, 0, INITIAL_BALANCE, INITIAL_BALANCE])
    });
}
...
fn test_get_reserve_amount() {
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        let reserve_balance = <Test as Config>::Currency::total_balance(&4_u64 ) - <Test as Config>::Currency::free_balance(&4_u64 );
