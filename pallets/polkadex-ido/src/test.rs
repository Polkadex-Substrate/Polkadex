...
fn test_register_investor() {
	...
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(ALICE.clone())), Ok(()));
		assert_noop!(
...
fn test_whitelist_investor() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(investor_address)), Ok(()));
...
fn test_participate_in_round() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(4_u64)), Ok(()));
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(2_u64)), Ok(()));
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(5_u64)), Ok(()));
...
fn test_claim_tokens() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(PolkadexIdo::claim_tokens(Origin::signed(investor_address), round_id,), Ok(()));

		assert_eq!(LastClaimBlockInfo::<Test>::contains_key(round_id, investor_address), true);

		assert_eq!(InfoClaimAmount::<Test>::contains_key(round_id, investor_address), true);
	});
}

#[test]
fn test_claim_tokens_native_pdex_as_token_b() {
	let balance: Balance = 100;
	let investor_address: u64 = 4;
	let funding_period = 10;
	let round_id = create_hash_data(&1u32);
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		// Investor invests 25 PDEX, will get 100% share (100 in Asset(24) tokens)  since 25 / 0.25
		// = 100
		assert_eq!(
			PolkadexIdo::show_interest_in_round(
				Origin::signed(investor_address),
				round_id,
				25_u128.saturated_into()
			),
			Ok(())
		);
		<PolkadexIdo as OnInitialize<u64>>::on_initialize(closing_block_number);
		...
		assert_eq!(<Test as Config>::AssetManager::balance(24, &investor_address), balance);
	});
}
...
fn test_claim_edge_case_small_investment() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert!(<Test as Config>::AssetManager::increase_balance(24, &ALICE, amount,).is_ok());
		...
		assert!(<Test as Config>::AssetManager::balance(24, &investor_address) > 0_u128)
	});
}
...
fn test_show_interest_in_round() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let round_id = <InfoProjectTeam<Test>>::get(ALICE.clone()).unwrap();
		assert!(PolkadexIdo::approve_ido_round(Origin::signed(1_u64), round_id).is_ok());
		let funding_round = <WhitelistInfoFundingRound<Test>>::get(round_id).unwrap();
...
fn test_show_interest_in_round_oversubscription() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let round_id = <InfoProjectTeam<Test>>::get(ALICE.clone()).unwrap();
		assert!(PolkadexIdo::approve_ido_round(Origin::signed(1_u64), round_id).is_ok());
		let investors: Vec<(u64, Balance)> = vec![(4u64, 200), (2u64, 200), (5u64, 200)];

		let funding_round: FundingRound<Test> =
			<WhitelistInfoFundingRound<Test>>::get(round_id).unwrap();
		...
		assert_eq!(PolkadexIdo::register_investor(Origin::signed(6_u64)), Ok(()));

		// Oversubsribed
		assert_eq!(
			PolkadexIdo::show_interest_in_round(Origin::signed(6_u64), round_id, 100),
...
fn test_withdraw_raise() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let round_id = <InfoProjectTeam<Test>>::get(ALICE).unwrap();
		let funding_round = <InfoFundingRound<Test>>::get(&round_id).unwrap();
		let open_block_number = funding_round.start_block;
		let closing_block_number = funding_round.close_round_block;
		assert!(PolkadexIdo::approve_ido_round(Origin::signed(1_u64), round_id).is_ok());
		...
		assert!(<Test as Config>::AssetManager::increase_balance(24, &4_u64, 100000,).is_ok());
		...
		//Test creator withdraw during when fundraising is not close should return error :
		// WithdrawalBlocked
		system::Pallet::<Test>::set_block_number(closing_block_number - 1);
		assert_noop!(
			PolkadexIdo::withdraw_raise(Origin::signed(ALICE), round_id, investor_address),
...
fn test_withdraw_token() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		//Test creator withdraw tokens during when fundraising is not close should return error :
		// WithdrawalBlocked
		system::Pallet::<Test>::set_block_number(closing_block_number - 1);
		assert_noop!(
			PolkadexIdo::withdraw_token(Origin::signed(ALICE), round_id, investor_address),
...
fn test_vote_for_round() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(WhitelistInfoFundingRound::<Test>::contains_key(round_id), true);
		assert_eq!(InfoFundingRound::<Test>::contains_key(round_id), false);
	});
}

#[test]
fn test_only_fetch_active_rounds() {
	let balance: Balance = 100;
	let funding_period: BlockNumber = 20;
	let cid = [0_u8; 32].to_vec();
	ExtBuilder::default().build().execute_with(|| {
		...
		let funding_round = <InfoFundingRound<Test>>::get(&round_id).unwrap();
		let open_block_number = funding_round.start_block;
		let close_block_number = funding_round.close_round_block;
		let yes_voters: Vec<u64> = [6, 7, 8, 9].to_vec();
		let no_voters: Vec<u64> = [4, 2, 5].to_vec();
		yes_voters.iter().for_each(|voter| {
			assert_eq!(
		...
		assert_eq!(WhitelistInfoFundingRound::<Test>::contains_key(round_id), true);
		system::Pallet::<Test>::set_block_number(open_block_number);
		let (active_round_id, _) = PolkadexIdo::active_rounds()[0];
		assert_eq!(active_round_id, round_id);

		system::Pallet::<Test>::set_block_number(close_block_number + 1);
		assert!(PolkadexIdo::active_rounds().is_empty(),);
	});
}
...
fn test_vote_for_round_no_vote_majority() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		<PolkadexIdo as OnInitialize<u64>>::on_initialize(open_block_number);
		assert_eq!(WhitelistInfoFundingRound::<Test>::contains_key(round_id), false);
		assert_eq!(InfoFundingRound::<Test>::contains_key(round_id), false);
	});
}
...
fn test_account_balances() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			PolkadexIdo::account_balances(vec![1, 2, 3, 5], ALICE),
			vec![INITIAL_BALANCE, 0, INITIAL_BALANCE, INITIAL_BALANCE]
		)
	});
}
...
fn test_get_reserve_amount() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		let reserve_balance = <Test as Config>::Currency::total_balance(&4_u64) -
			<Test as Config>::Currency::free_balance(&4_u64);
