...
fn test_transfer_native_asset() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            pending_withdrawal.to_vec().pop().unwrap(),
            approved_withdraw
        );
    })
}
...
fn test_set_withdrawal_fee_full() {
    new_test_ext().execute_with(|| {
        // bad origins
        assert_noop!(
            TheaExecutor::set_withdrawal_fee(RuntimeOrigin::none(), 1, 1),
            BadOrigin
        );
        assert!(<WithdrawalFees<Test>>::get(1).is_none());
        assert_noop!(
            TheaExecutor::set_withdrawal_fee(RuntimeOrigin::signed(1), 1, 1),
            BadOrigin
        );
        assert!(<WithdrawalFees<Test>>::get(1).is_none());
        // proper origin
...
fn test_update_asset_metadata_full() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            TheaExecutor::update_asset_metadata(RuntimeOrigin::none(), 1, 1),
            BadOrigin
        );
        // invalid decimal
        assert_noop!(
...
fn test_deposit_without_account() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            999_999_994_984_954u128
        );
        assert_eq!(
            Assets::balance(asset_id, &TheaExecutor::thea_account()),
            0u128
        );
        assert_eq!(
...
fn test_claim_deposit_returns_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(TheaExecutor::do_deposit(1, &vec![deposit].encode()));
        assert_ok!(TheaExecutor::claim_deposit(
