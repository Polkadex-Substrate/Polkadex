...
use thea_primitives::parachain::{ApprovedWithdraw, ParachainWithdraw, ParachainAsset, AssetType};
...
fn test_withdraw_with_wrong_benificiary_length() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
			TheaExecutor::withdraw(RuntimeOrigin::signed(1), 1u128, 1000u128, beneficiary.to_vec(), false),
...
fn claim_deposit_pass_with_proper_inputs() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(TheaExecutor::claim_deposit(Origin::signed(1), 100), Error::<Test>::NoApprovedDeposit);
