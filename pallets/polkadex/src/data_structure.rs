...
// TODO [Doubt] @Krishna Gautham please have a look ; Check if encode thow any error if not remove option
    ...
    pub fn u128_to_vec(x: T::Balance) -> Option<Vec<u8>> {
        Some(x.encode())
    }
}
...
pub struct LinkedPriceLevel<T> where T: Trait {
    pub next: Option<T::Balance>,
    pub prev: Option<T::Balance>,
...
pub struct Orderbook<T> where T: Trait {
    ...
    pub best_bid_price: T::Balance,
    pub best_ask_price: T::Balance,
}
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, Debug)]
pub struct MarketData<T> where T: Trait {
...
impl<T> MarketData<T> where T: Trait {
...
impl<T> Default for MarketData<T> where T: Trait {
    fn default() -> Self {
        MarketData {
            low: 0.into(),
            high: 0.into(),
