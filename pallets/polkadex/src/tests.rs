...
fn setup_balances() {
    ...
    let options_bob = pallet_generic_asset::AssetOptions::<u128, u64> {
        initial_issuance: 105 * UNIT/100, // Made changes to accommodate additional limit orders
        permissions: Default::default(),
    };
...
fn check_trading_engine() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(pallet_generic_asset::Module::<Test>::free_balance(&1, &alice), ((UNIT / 1000) * 375000));
        assert_eq!(pallet_generic_asset::Module::<Test>::reserved_balance(&1, &alice), ((UNIT / 1000) * 2030000));
