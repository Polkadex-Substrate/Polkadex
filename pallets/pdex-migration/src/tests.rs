...
pub fn set_relayer_status_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PDEXMigration::set_relayer_status(
            RuntimeOrigin::root(),
            relayer,
            true
        ));
...
pub fn remove_minted_tokens_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            MintableTokens::<Test>::get(),
            3_172_895 * PDEX - valid_amount
        );
        assert_eq!(
        ...
        assert_ok!(PDEXMigration::remove_minted_tokens(
            RuntimeOrigin::root(),
            beneficiary
        ));
...
pub fn mint_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            PDEXMigration::unlock(RuntimeOrigin::signed(beneficiary)).is_err(),
            true
        );
        ...
        // Check balances
        assert_eq!(
            pallet_balances::Pallet::<Test>::free_balance(100),
            99 * PDEX
        );
        assert_eq!(
            pallet_balances::Pallet::<Test>::free_balance(beneficiary),
            1 * PDEX
        );
    });
}
