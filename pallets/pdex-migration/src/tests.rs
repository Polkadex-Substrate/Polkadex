...
pub fn unlock_tokens_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(PDEXMigration::mint(Origin::signed(relayer1), beneficiary,valid_amount,eth_hash));
		...
		assert_noop!(PDEXMigration::unlock(Origin::signed(unknown_beneficiary)), Error::<Test>::UnknownBeneficiary);
		assert_noop!(PDEXMigration::unlock(Origin::signed(beneficiary)), Error::<Test>::LiquidityRestrictions);
	})
}
...
pub fn remove_minted_tokens_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(PDEXMigration::unlock(Origin::signed(beneficiary)), Error::<Test>::UnknownBeneficiary);
...
pub fn mint_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
            pallet_balances::Pallet::<Test>::transfer(Origin::signed(beneficiary),100,valid_amount-1*PDEX), // minus 1 PDEX is because of existential deposit
		...
		frame_system::Pallet::<Test>::set_block_number(frame_system::Pallet::<Test>::current_block_number() + 201600);
		// Unlock tokens
		assert_ok!(PDEXMigration::unlock(Origin::signed(beneficiary)));
		// check if it is transferable
		assert_ok!(pallet_balances::Pallet::<Test>::transfer(Origin::signed(beneficiary),100,valid_amount-1*PDEX));
		// Check balances
