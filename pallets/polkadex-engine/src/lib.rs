...
use sp_runtime::traits::{Hash, SignedExtension, DispatchInfoOf, Bounded};
...
decl_error! {
	pub enum Error for Module<T: Config> {
		...
		OverFlowError,
		/// ErrorIsNotMarketMaking
		ErrorIsNotMarketMaking,
		/// Wrong Order type
		WrongOrderType,
		/// Not enough Polkadex Balance
		InsufficientPolkadexBalance,
		/// Uniswap registration failed
		UniswapRegistrationFailed,
		/// Corresponding Polkadex pair should be registered first
		CorrespondingPolkadexMarketNotAvailable,
	}
}
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        ///
        /// * `base_asset_id` - It will be native asset
        ///
        /// # Return
        ///
		...
		pub fn register_new_orderbook_with_polkadex(origin, quote_asset_id: T::Hash, new_token_deposit: T::Balance) -> dispatch::DispatchResultWithPostInfo{
		    ...
		    ensure!(!(&quote_asset_id == &native_currency), <Error<T>>::SameAssetIdsError);

		    // Checks the tradingPair whether exists
		    let trading_pair_id = Self::get_pair(quote_asset_id, native_currency);
		    ...
		    Self::create_order_book(trading_pair_id.0, trading_pair_id.1, trading_pair_id);
		...
		pub fn register_new_orderbook(origin, quote_asset_id: T::Hash, quote_token_deposit: T::Balance, base_asset_id: T::Hash, base_token_deposit: T::Balance) -> dispatch::DispatchResultWithPostInfo{
		    ...
		    // Checks the tradingPair whether exists
		    let trading_pair_id = Self::get_pair(quote_asset_id, base_asset_id);
		    ensure!(!<Orderbooks<T>>::contains_key(&trading_pair_id), <Error<T>>::TradingPairIDExists);
		    ensure!(<Orderbooks<T>>::contains_key(Self::get_pair(quote_asset_id,native_currency)), <Error<T>>::CorrespondingPolkadexMarketNotAvailable);
		    ...
		    ensure!(Self::register_on_uniswap(&trader, quote_asset_id,quote_token_deposit,base_asset_id,base_token_deposit), <Error<T>>::UniswapRegistrationFailed);
		    // TODO: Once the treasury pallet is installed transfer the trading pair registration fee to treasury.
		    Self::create_order_book(trading_pair_id.0, trading_pair_id.1, trading_pair_id);
		    Self::deposit_event(RawEvent::TradingPairCreated(trading_pair_id.0, trading_pair_id.1));
		    Ok(Some(0).into())
	    }
...
impl<T: Config> Module<T> {
    ...
    fn register_on_uniswap(trader: &<T as frame_system::Config>::AccountId, quote_asset_id: T::Hash, quote_token_deposit: T::Balance, base_asset_id: T::Hash, base_token_deposit: T::Balance) -> bool {
        ...
        if trading_pair.0 == base_asset_id {
            polkadex_swap_engine::Module::<T>::do_register_swap_pair(trader,
                                                                     ...
                                                                     trading_pair.1,
                                                                     base_token_deposit,
                                                                     quote_token_deposit).is_ok()
        } else {
            polkadex_swap_engine::Module::<T>::do_register_swap_pair(trader,
                                                                     trading_pair.0,
                                                                     trading_pair.1,
                                                                     quote_token_deposit,
                                                                     base_token_deposit).is_ok()
        }
    }
    ...
    fn create_order_book(quote_asset_id: T::Hash, base_asset_id: T::Hash, trading_pair_id: (T::Hash, T::Hash)) {
            ...
            Ok(mut orderbook) => {
                ...
                match current_order.order_type {
                    ...
                    OrderType::AskLimit | OrderType::BidLimit => {
                        if (current_order.order_type == OrderType::BidLimit &&
                            ...
                            if current_order.quantity > FixedU128::from(0) {
                                let status = polkadex_swap_engine::Module::<T>::swap_by_orderbook(&current_order.trader,
                                                                                                  current_order.trading_pair,
                                                                                                  current_order.order_type.convert().ok_or(Error::<T>::WrongOrderType.into())?,
                                                                                                  *&current_order.price,
                        ...
                        } else if polkadex_swap_engine::Module::<T>::swap_by_orderbook(&current_order.trader,
                                                                                       current_order.trading_pair,
                                                                                       if current_order.order_type == OrderType::BidLimit { true } else { false },
                                                                                       current_order.price,
                                                                                       current_order.quantity,
                                                                                       orderbook.quote_asset_id) {
                            // TODO: Emit event regarding Swap by Orderbook
                        } else {
                            Self::insert_order(&current_order, &mut orderbook)?;
                        }
                    }

                    OrderType::AskLimitMM | OrderType::BidLimitMM => {
                        if (current_order.order_type == OrderType::BidLimitMM &&
                            current_order.price >= orderbook.best_ask_price &&
                            orderbook.best_ask_price != FixedU128::from(0)) ||
                            (current_order.order_type == OrderType::AskLimitMM &&
                                current_order.price <= orderbook.best_bid_price &&
                                orderbook.best_bid_price != FixedU128::from(0)) {
                            Self::consume_order(&mut current_order, &mut orderbook)?;
                    ...
                    OrderType::AskLimitMMOnly | OrderType::BidLimitMMOnly => {
                        if (current_order.order_type == OrderType::BidLimitMMOnly &&
                            current_order.price >= orderbook.best_ask_price &&
                            orderbook.best_ask_price != FixedU128::from(0)) ||
                            (current_order.order_type == OrderType::AskLimitMMOnly &&
                                current_order.price <= orderbook.best_bid_price &&
                                orderbook.best_bid_price != FixedU128::from(0)) {
                            Err(<Error<T>>::ErrorIsNotMarketMaking.into())?;
                        } else {
                            Self::insert_order(&current_order, &mut orderbook)?;
                        }
                    }
    ...
    fn insert_order(current_order: &Order<T>, orderbook: &mut Orderbook<T>) -> Result<(), Error<T>> {
        match current_order.order_type {
            OrderType::BidLimit | OrderType::BidLimitMM | OrderType::BidLimitMMOnly => {
            ...
            OrderType::AskLimit | OrderType::AskLimitMM | OrderType::AskLimitMMOnly => {
    ...
    fn consume_order(current_order: &mut Order<T>, orderbook: &mut Orderbook<T>) -> Result<(), Error<T>> {
        ...
        match current_order.order_type {
            OrderType::BidLimit | OrderType::BidLimitMM => {
            ...
            OrderType::AskLimit | OrderType::AskLimitMM => {
    ...
    fn basic_order_checks(order: &Order<T>) -> Result<Orderbook<T>, Error<T>> {
        match order.order_type {
            OrderType::BidLimit | OrderType::AskLimit | OrderType::AskLimitMM | OrderType::BidLimitMM | OrderType::AskLimitMMOnly | OrderType::BidLimitMMOnly if order.price <= FixedU128::from(0) || order.quantity <= FixedU128::from(0) => Err(<Error<T>>::InvalidPriceOrQuantityLimit.into()),
            OrderType::BidMarket if order.price <= FixedU128::from(0) => Err(<Error<T>>::InvalidBidMarketPrice.into()),
            OrderType::BidMarket | OrderType::BidLimit | OrderType::BidLimitMM | OrderType::BidLimitMMOnly => Self::check_order(order),

            OrderType::AskMarket if order.quantity <= FixedU128::from(0) => Err(<Error<T>>::InvalidAskMarketQuantity.into()),
            OrderType::AskMarket | OrderType::AskLimit | OrderType::AskLimitMM | OrderType::AskLimitMMOnly => Self::check_order(order),
        }
    }
    ...
    fn check_order(order: &Order<T>) -> Result<Orderbook<T>, Error<T>> {
        ...
        let balance: <T>::Balance = match order.order_type {
            OrderType::BidLimit | OrderType::BidMarket | OrderType::BidLimitMM | OrderType::BidLimitMMOnly => polkadex_custom_assets::Module::<T>::free_balance(&order.trader, orderbook.base_asset_id),
            OrderType::AskMarket | OrderType::AskLimit | OrderType::AskLimitMM | OrderType::AskLimitMMOnly => polkadex_custom_assets::Module::<T>::free_balance(&order.trader, orderbook.quote_asset_id),
        };

        match Self::convert_balance_to_fixed_u128(balance) {
            Some(converted_balance) if order.order_type == OrderType::BidLimit || order.order_type == OrderType::BidLimitMM || order.order_type == OrderType::BidLimitMMOnly => {
            ...
            Some(converted_balance) if (order.order_type == OrderType::AskLimit || order.order_type == OrderType::AskMarket || order.order_type == OrderType::AskLimitMM || order.order_type == OrderType::AskLimitMMOnly) && converted_balance < order.quantity => Err(<Error<T>>::InsufficientAssetBalance.into()),
            ...
            Some(_) if order.order_type == OrderType::AskLimitMM => Ok(orderbook),
            Some(_) if order.order_type == OrderType::AskLimitMMOnly => Ok(orderbook),
    ...
    fn reserve_user_balance(orderbook: &Orderbook<T>, order: &Order<T>, amount: FixedU128) -> Result<(), Error<T>> {
        ...
        let asset_id = if order.order_type == OrderType::AskLimit || order.order_type == OrderType::AskLimitMM || order.order_type == OrderType::AskLimitMMOnly
        { orderbook.quote_asset_id } else { orderbook.base_asset_id };
        ...
        match call.is_sub_type() {
            Some(Call::submit_order(order_type,trading_pair,price,quantity)) => {
                ...
                match order_type {
                    ...
                    OrderType::BidMarket => Self::check_bid_market(who,trading_pair_id, price)?,
                    OrderType::AskLimit | OrderType::AskMarket | OrderType::AskLimitMM | OrderType::AskLimitMMOnly => Self::check_ask_type_order(who,trading_pair_id, quantity)?
                }
