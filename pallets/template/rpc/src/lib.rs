...
pub trait DexStorageApi<BlockHash, K> where K: Trait {
    ...
    fn get_price_level(&self, at: Option<BlockHash>, trading_pair: H256) -> Result<Vec<LinkedPriceLevelRpc<K>>>;
}
        ...
        C::Api: DexStorageRuntimeApi<Block, K>,
{
    ...
    fn get_price_level(&self, _at: Option<<Block as BlockT>::Hash>, trading_pair: H256) -> Result<Vec<LinkedPriceLevelRpc<K>>> {
        let api = self.client.runtime_api();
        let at = BlockId::hash(
            // Always take the best block hash for this RPC
            self.client.info().best_hash);
