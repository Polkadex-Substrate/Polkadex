...
impl<AccountId: Codec + Clone + TypeInfo> WithdrawalRequest<AccountId> {
    ...
    pub fn verify(&self) -> bool {
        ...
        self.signature
            .verify(self.payload.encode().as_ref(), &signer)
    }
...
impl TryFrom<String> for TradingPair {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        ...
        let base_asset = if assets[0] == String::from("PDEX").as_str() {
            ...
            let id = assets[0]
                .parse::<u128>()
                .map_err(|_| "asset id parse error")?;
            AssetId::Asset(id)
        };
...
impl Ord for OrderKey {
    fn cmp(&self, other: &Self) -> Ordering {
        assert_eq!(
            ...
            "Comparison cannot work for opposite order sides"
        );
